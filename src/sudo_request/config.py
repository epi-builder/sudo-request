...
class Config:
    ...
    approval_wait_heartbeat_seconds: int = 10
...
def load_config(home: Path) -> Config:
    ...
    if heartbeat <= 0:
        raise ValueError("approval_wait_heartbeat_seconds must be positive")
    if window_default <= 0:
    ...
    return Config(token_file, allowed, approval_timeout, heartbeat, window_default, window_max)
