...
from sudo_request.cli import command_run, command_update_itself, ipc_request_with_heartbeat
...
class CliTests(unittest.TestCase):
    ...
    def test_command_update_itself_wraps_install_command(self) -> None:
        with patch("sudo_request.cli.update_itself_command", return_value=["/usr/bin/sudo", "/usr/bin/python3", "-m", "sudo_request", "install"]):
            with patch("sudo_request.cli.command_run", return_value=0) as run:
